...
$class_reservations_query = "SELECT cr.user_id, u.username AS user_name, c.training_id, t.name AS training_name, cr.class_id, cr.status 
...
$personal_training_reservations_query = "SELECT ptr.registration_id, ptr.user_id, u.username AS user_name, ptr.status, pt.training_name 
                                         FROM personaltrainingregistration ptr
                                         INNER JOIN personaltraining pt ON ptr.training_id = pt.id 
                                         INNER JOIN user u ON ptr.user_id = u.user_id
                                         WHERE pt.trainer_id = ? AND ptr.status <> 'approved'";
$stmt = mysqli_prepare($con, $personal_training_reservations_query);
mysqli_stmt_bind_param($stmt, "i", $trainer_id);
mysqli_stmt_execute($stmt);
$personal_training_reservations_result = mysqli_stmt_get_result($stmt);
...
$approved_personal_training_registrations_query = "SELECT ptr.user_id, u.username AS user_name, ptr.status 
                                                   ...
                                                   INNER JOIN user u ON ptr.user_id = u.user_id
                                                   INNER JOIN personaltraining pt ON ptr.registration_id = pt.id
                                                   WHERE pt.trainer_id = ?";
...
<table border="1">
    ...
    <?php while ($row = mysqli_fetch_assoc($personal_training_reservations_result)) : ?>
        <tr>
            <td><?php echo $row['user_name']; ?></td>
            <td><?php echo $row['training_name']; ?></td>
            <td><?php echo $row['status']; ?></td>
            <td>
                <form method="POST" action="">
                    <input type="hidden" name="training_id" value="<?php echo $row['registration_id']; ?>">
